import { refundTerms } from '../commerce/commerce.policy';
...
export class OrdersService {
  ...
  async purchase(userId: number, key: string, dto: CreateGpOrderDto) {
    ...
    return this.database.transaction(async (manager) => {
      ...
      const [{ reserved }] = await manager.query(reservedStockSql, [gacha.id]);
      ...
      const terms = refundTerms(createdAt, gacha.saleType || 'UNSPECIFIED');
      ...
      const order = await orders.save(
        orders.create({
          createdAt,
          refundEligible: terms.eligible,
          refundUntil: terms.until,
          refundPolicy: terms.policy,
