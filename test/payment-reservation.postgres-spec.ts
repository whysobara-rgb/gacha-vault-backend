...
// Each suite execution owns its database. Before/after regression runs cannot
// collide with records left by the earlier integration suite.
const databaseName = 'gacha_payment_' + randomUUID().replace(/-/g, '');
const local = {
  ...
  synchronize: false, migrationsRun: false, dropSchema: false,
  extra: { max: 60, statement_timeout: 12000, connectionTimeoutMillis: 12000 },
};
const admin = new DataSource({ ...local, database: 'postgres', extra: { max: 2 } });
const db = new DataSource({ ...dataSourceOptions, ...local, database: databaseName });
...
describe('card reservation expiry and recovery safety (mock PG)', () => {
  ...
  async function fixture(stock = 1) {
    ...
    return { owner, buyer, g: g.id, transactionId: randomUUID().replace(/-/g, ''), dto: {
  ...
  it('uses the DB clock: an expired reservation cannot revive after another buyer owns the last stock', async () => {
    ...
    const result = await observed(payments.confirm(f.owner, p.paymentId, f.transactionId, 100));
  ...
  it('rechecks expiry after a real payment-row lock wait before dispatching to the PG', async () => {
    ...
    try {
      ...
      approval = observed(payments.confirm(f.owner, p.paymentId, f.transactionId, 100));
  ...
  it('retains CONFIRMING and UNKNOWN reservations after expiry without redispatch or cancellation', async () => {
    ...
    const running = observed(payments.confirm(f.owner, p.paymentId, f.transactionId, 100));
    try {
      ...
      const inProgress = await payments.confirm(f.owner, p.paymentId, f.transactionId, 100);
      ...
      expect((await payments.confirm(f.owner, p.paymentId, f.transactionId, 100)).status).toBe('UNKNOWN');
  ...
  it('cancels a prepared reservation idempotently and does not revive it on a late callback', async () => {
    ...
    await orders.purchase(f.buyer, randomUUID(), f.dto);
    await expect(payments.confirm(f.owner, p.paymentId, f.transactionId, 100)).rejects.toMatchObject({ status: 409 });
    expect(confirm).not.toHaveBeenCalled();
    expect(await orderCount(f.g)).toBe(1);
  });
  ...
  it('recovers an approved payment after issuance rollback without a second PG approval', async () => {
    ...
    try {
      ...
      await expect(payments.confirm(f.owner, p.paymentId, f.transactionId, 100)).rejects.toThrow('synthetic payment issuance failure');
    ...
    const recovered = await Promise.all(Array.from({ length: 6 }, () => payments.confirm(f.owner, p.paymentId, f.transactionId, 100)));
  ...
  it('rejects wrong owners and mismatched amount before calling the provider', async () => {
    const f = await fixture();
    const p = await payments.prepare(f.owner, randomUUID(), f.dto);
    await expect(payments.confirm(f.buyer, p.paymentId, f.transactionId, 100)).rejects.toMatchObject({ status: 404 });
    await expect(payments.confirm(f.owner, p.paymentId, f.transactionId, 200)).rejects.toMatchObject({ status: 409 });
    expect(confirm).not.toHaveBeenCalled();
